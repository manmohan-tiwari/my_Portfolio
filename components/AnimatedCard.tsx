...
export default function AnimatedCard({
  ...
  return (
    ...
    >
      {/* Top Border on Hover */}
      <div className="absolute top-0 left-0 w-full h-[3px] bg-teal-500 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
      ...
      {(label || title) && (
        <div className="mb-6 flex flex-col gap-2">
          ...
          {title && (
            <h3 className="text-2xl font-serif font-bold text-slate-900 dark:text-white group-hover:text-teal-600 dark:group-hover:text-teal-400 transition-colors">
              {title}
            </h3>
          )}
        </div>
